...
static tOptimaly      tOpt;
static tCovariance   tCov;
//float  Z[LENGTH]  = {0};           //  测量值(每次测量的数据需要存入该数组)
static float         I[LENGTH]  = {1};              //  单位矩阵
static float         X[LENGTH]  = {0};              //  当前状态的预测值
static float         P[LENGTH]  = {1};              //  当前状态的预测值的协方差
...
static float         Q[LENGTH]  = {25};//0.0001f      //  系统过程的协方差	协方差的定义：真实值与期望值之差的平方的期望值
static float         R[LENGTH]  = {225};              //  测量过程的协方差	协方差的定义：真实值与期望值之差的平方的期望值   
...
static float         Temp2[LENGTH] = {1};       //  辅助变量, 同时保存tCov.PPreOpt[]的初始化值


void KalMan_PramInity(void)
{
  unsigned char   i;
